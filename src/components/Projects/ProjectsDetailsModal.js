...
export default function ProjectsDetailsModal(props) {
  ...
  const {id,description, gitLink, allImages,languageInfo ,name,liveLink} = projectArray;
  useEffect(() => {
    axios.get(`https://portfolio-backend-nu-dusky.vercel.app/projects/${props.id}`)
      .then((response) => {
  ...
  >
    ...
    <Modal.Body style={{background:'rgb(20 15 35)'}}>
      ...
      <p style={{color:"#fff"}}>
      <span style={{fontSize:'18px' , fontWeight:'bold'}}>Project Description</span> : {description}
      </p>
      <p style={{color:"#fff"}}>
...
<ul>
    <li>{info}</li>
      ...
      {
            gitLink && (
              <Button variant="primary" href={gitLink} target="_blank" style={{width:'200px'}}>
              <BsGithub /> &nbsp;
               GitHub
            </Button>
            )
    ...
    <Modal.Footer style={{background:'rgb(20 15 35)'}}>
      <Button onClick={props.onHide} style={{color:"#fff"}}>Close</Button>
    </Modal.Footer>
  </Modal>
  )
}
